// outline_diagram_export.fsh
// Identical to simulated:contraption_diagram/outline_diagram.fsh except that
// sdScene() no longer subtracts the on-screen button and rotation-gizmo
// regions — those only exist during live in-game rendering, not during export.
...
// Export variant: only the main vignette box, no button / rotation-gizmo cutouts.
// The original sdScene() subtracted two hard-coded screen-space regions (the
// force-button column and the rotation gizmo) so that the rendered scene does
// not bleed through the UI controls.  During export there is no UI overlay, so
// those cutouts must be omitted — otherwise the corresponding areas in the
// exported image will be incorrectly transparent.
float sdScene(vec2 pos, vec2 size) {
    float fade_scale = 1.0 / FadeScale;
    float box = sdBox(pos - (size / 2), size / (vec2(2.3, 2.4)) * fade_scale);
    return box;
}
